...
$order_array = [];
    foreach ($customer_orders as $customer_order) 
	{
		...
		foreach ( $order_items as $item )
		{
			...
			{
				$product_name_text = esc_html__( 'Wallet Amount', 'khebrat_theme' );
			}
...
{
	$main_heading = esc_html__('Deposit and Invoices','khebrat_theme');
}
else
{
	$main_heading = esc_html__('Invoices','khebrat_theme');
}
?>
				...
				<div class="card-body">
				  <div class="pro-section">
					  <div class="pro-box heading-row">
						<div class="pro-coulmn no-flex-grow"><?php echo esc_html__( 'Invoice ID', 'khebrat_theme' ) ?> </div>
						<div class="pro-coulmn"><?php echo esc_html__( 'Amount', 'khebrat_theme' ) ?> </div>
						<div class="pro-coulmn"><?php echo esc_html__( 'Payment Status', 'khebrat_theme' ) ?> </div>
						<div class="pro-coulmn"><?php echo esc_html__( 'Detail', 'khebrat_theme' ) ?> </div>
					  </div>
						<?php
									...
									?>
									  <div class="pro-box">
										...
										<div class="pro-coulmn">
											<?php
											$badge_color ='';
											if( $array['status'] == 'completed') { $badge_color = 'btn-inverse-success'; $status_text = esc_html__( 'Completed', 'khebrat_theme' );}
											else if($array['status'] == 'processing'){ $badge_color = 'btn-inverse-primary'; $status_text = esc_html__( 'Processing', 'khebrat_theme' );}
											else if($array['status'] == 'pending'){ $badge_color = 'btn-inverse-warning'; $status_text = esc_html__( 'Pending', 'khebrat_theme' );}
											else if($array['status'] == 'on-hold'){ $badge_color = 'btn-inverse-dark'; $status_text = esc_html__( 'On Hold', 'khebrat_theme' );}
											else if($array['status'] == 'cancelled'){ $badge_color = 'btn-inverse-secondary'; $status_text = esc_html__( 'Cancelled', 'khebrat_theme' );}
											else if($array['status'] == 'refunded'){ $badge_color = 'btn-inverse-info'; $status_text = esc_html__( 'Refunded', 'khebrat_theme' );}
											else if($array['status'] == 'failed'){ $badge_color = 'btn-inverse-danger'; $status_text = esc_html__( 'Failed', 'khebrat_theme' );}
											?>
											<span class="badge btn <?php echo esc_html($badge_color); ?>">
											<?php echo wp_return_echo($status_text); ?>
											</span>
										</div>
										<div class="pro-coulmn"><a href="<?php get_template_part('');?>?ext=invoice-detail&invoice-id=<?php echo esc_html($array['ID']); ?>" class="btn btn-secondary"><?php echo esc_html__( 'View Invoice', 'khebrat_theme' ); ?></a></div>
									  </div>
							...
							else
							{
								...
								<div class="nothing-found">
									<h3><?php echo esc_html__( 'Sorry!!! No Invoice Found', 'khebrat_theme' ) ?></h3>
