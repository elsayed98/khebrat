...
{
	if(isset($khebrat_theme_options['user_registration_type_selection']) && count(array_filter($khebrat_theme_options['user_registration_type_selection'])) > 1 )
	{
		if($emp_id == '' && $fre_id == '')
		{
			?>
			  ...
			  <div class="modal-dialog" role="document">
				<div class="modal-content">
				  <div class="modal-header">
					<h4 class="modal-title"><?php echo esc_html__('Select User Type','khebrat_theme');?></h4>
				  </div>
				  <div class="modal-body">
					<form id="user-selection-form">
						<div class="form-group">
							...
							<p><?php echo esc_html__('You must have to select one option to move forward.','khebrat_theme'); ?></p>
						</div>
						<div class="form-group"> <button type="button" id="set-user-type" class="btn btn-theme-secondary btn-loading btn-block" ><?php echo esc_html__('Submit & Save','khebrat_theme'); ?> <div class="bubbles"> <i class="fa fa-circle"></i> <i class="fa fa-circle"></i> <i class="fa fa-circle"></i> </div></button> </div>
					</form>
				  </div>
				</div>
			  </div>
				...
				<div class="modal-content">
				  ...
				  <div class="modal-body">
					<form id="user-selection-form-update">
						...
						<p><?php echo esc_html__('You must have to select one option to move forward.','khebrat_theme'); ?></p>
						<p><?php echo esc_html__('Please select this option carefully as you will not be able to change it in the future.','khebrat_theme'); ?></p>
						<div class="form-group"> <button type="button" id="set-user-type-update" class="btn btn-theme-secondary btn-loading btn-block" ><?php echo esc_html__('Submit & Save','khebrat_theme'); ?> <div class="bubbles"> <i class="fa fa-circle"></i> <i class="fa fa-circle"></i> <i class="fa fa-circle"></i> </div></button> </div>
					</form>
				  </div>
				</div>
			  </div>
...
<nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
        ...
        <div class="navbar-brand-inner-wrapper d-flex justify-content-between align-items-center w-100">  
          <a class="navbar-brand brand-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>"><img src="<?php echo esc_url($khebrat_theme_options['dasboard_logo']['url']); ?>" alt="<?php echo get_post_meta($img_id, '_wp_attachment_image_alt', TRUE); ?>"/></a>
          <a class="navbar-brand brand-logo-mini" href="<?php echo esc_url( home_url( '/' ) ); ?>"><img src="<?php echo esc_url($khebrat_theme_options['dasboard_logo']['url']); ?>" alt="logo"/></a>
			...
			if(isset($khebrat_theme_options['exertio_notifications']) && $khebrat_theme_options['exertio_notifications'] == true)
			{
				...
				<li class="nav-item dropdown notification-click">
					...
					<div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="profileDropdown">
						<h5 class="notification-title"><?php echo esc_html__('Notifications', 'khebrat_theme' ); ?></h5>
						<div class="notification-list"><?php echo exertio_get_notifications($current_user_id); ?></div>
						<p class="notification-view-all"><a href="<?php echo esc_url(get_the_permalink());?>?ext=notifications"> <?php echo esc_html__('See all notifications', 'khebrat_theme' ); ?></a></p>
					</div>
				</li>
				<?php
			}
            ...
            <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="profileDropdown">
                ...
                if(isset($is_wallet_active) && $is_wallet_active == 0)
					{
						...
						<li class="dropdown-item wallet-contanier">
							<div>
								<span class="text"> <?php echo esc_html__('Wallet Funds','khebrat_theme'); ?></span>
								<h4>
					...
					<a class="dropdown-item profile_selection  <?php if(isset($active_profile) && $active_profile == 2) { echo 'selected'; } ?>" data-profile-active="freelancer" href="javascript:void(0)">
						<span class="profile-img">
							<?php
								...
								{
									?>
									<img src="<?php echo esc_url($khebrat_theme_options['freelancer_df_img']['url']); ?>" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>" class="img-fluid">
									<?php	
								}
							?>
						</span>
						<span>
							...
							<p><?php echo esc_html__('Freelancer','khebrat_theme'); ?></p>
						</span>
					</a>
				</li>
				<li>
					<a class="dropdown-item <?php if(isset($active_profile) && $active_profile == 1) { echo 'selected'; } else if(empty($active_profile)) { echo 'selected'; } ?> profile_selection" data-profile-active="employer" href="javascript:void(0)">
						<span class="profile-img">
							<?php
								...
								{
									?>
									<img src="<?php echo esc_url($khebrat_theme_options['employer_df_img']['url']); ?>" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>" class="img-fluid">
									<?php	
								}
							?>
						</span>
						<span>
							...
							<p><?php echo esc_html__('Employer','khebrat_theme'); ?></p>
						</span>
					</a>
				</li>
				<?php
				...
				<li>
				  <a class="dropdown-item" href="<?php echo esc_url(get_the_permalink());?>?ext=edit-profile">
					...
					<?php echo esc_html__('Edit Profile','khebrat_theme'); ?>
				  </a>
				</li>
				<li>
					<a href="<?php echo wp_logout_url( get_the_permalink( $khebrat_theme_options['login_page'] ) ); ?>" class="dropdown-item">
						<i class="mdi mdi-logout text-primary"></i>
						<?php echo esc_html__('Logout','khebrat_theme'); ?>
					</a>
				  </li>
				  </ul>
            </div>
