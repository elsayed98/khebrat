...
$header_cpts = $khebrat_theme_options['header_searchbar_dropdown'];
foreach($header_cpts as $cpts)
{
	if($cpts == 'project')
	{
		$cpt_list .= '<li data-cpt ="project">'.esc_html__('Find Job', 'khebrat_theme').' </li>';
		$cpt_link = get_the_permalink($khebrat_theme_options['project_search_page']);
		$cpt_placehoder =  __('Find Job', 'khebrat_theme');
	}
	if($cpts == 'services')
	{
		$cpt_list .= '<li data-cpt ="service">'.esc_html__('Get Job Done', 'khebrat_theme').' </li>'; 
		$cpt_link = get_the_permalink($khebrat_theme_options['services_search_page']);
		$cpt_placehoder =  __('Get Job Done', 'khebrat_theme');
	}
	if($cpts == 'freelancer')
	{
		$cpt_list .= '<li data-cpt ="freelancer">'.esc_html__('Find Talent', 'khebrat_theme').' </li>';
		$cpt_link = get_the_permalink($khebrat_theme_options['freelancer_search_page']);
		$cpt_placehoder =  __('Find Talent', 'khebrat_theme');
	}
	if($cpts == 'employer')
	{
		$cpt_list .= '<li data-cpt ="employer">'.esc_html__('Search Employer', 'khebrat_theme').' </li>'; 
		$cpt_link = get_the_permalink($khebrat_theme_options['employer_search_page']);
		$cpt_placehoder =  __('Search Employer', 'khebrat_theme');
	}
}
?>
<!-- Header -->
      ...
      <div class="container-fluid"> 
        ...
        <div class="sb-header-container"> 
          ...
          <nav class="sb-menu menu-caret submenu-top-border submenu-scale">
            <ul>
            	       <?php if(isset($khebrat_theme_options['exertio_notifications']) && $khebrat_theme_options['exertio_notifications'] == true)
                {
                    if(is_user_logged_in()){
                        ...
                        ?>
                            ...
                            <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="dropdownMenuButton1">
                                ...
                                <p class="notification-view-all"><a href="<?php echo esc_url(home_url());?>/dashboard/?ext=notifications"> <?php echo esc_html__('See all notifications', 'khebrat_theme' ); ?></a></p>
                            </div>
                        </li>
                        <?php
                    }
			  ...
			  $freelancer_btn_text_login = fl_framework_get_options('freelancer_btn_text_login');
                if( is_user_logged_in() )
                {
                    ...
                    <ul>
                        ...
                        <li> <a class="dropdown-item" href="<?php echo esc_url(get_the_permalink($dashboard_page)); ?>"><?php echo esc_html__('Dashboard','khebrat_theme'); ?></a> </li>
                        <li> <a class="dropdown-item" href="<?php echo esc_url(get_the_permalink($dashboard_page));?>?ext=edit-profile"><?php echo esc_html__('Edit Profile','khebrat_theme'); ?></a> </li>
                        <li> <a class="dropdown-item" href="<?php echo wp_logout_url(get_the_permalink(fl_framework_get_options('login_page'))); ?>"><?php echo esc_html__('Logout','khebrat_theme'); ?></a> </li>
                    </ul>
                    </li>
                    <?php
                }
...
<?php
	$header_cats = $khebrat_theme_options['category_bar'];
if(isset($header_cats) && $header_cats == 1)
{
	$header_categor_bar = $listings_link = '';
	$header_category_cpt = isset($khebrat_theme_options['header_category_cpt'])?$khebrat_theme_options['header_category_cpt'] : '' ;
	if(isset($header_category_cpt) && $header_category_cpt == 'project')
	{
		$header_categor_bar = $khebrat_theme_options['header_categor_bar_project'];
	...
	{
		$header_categor_bar = $khebrat_theme_options['header_categor_bar_services'];
