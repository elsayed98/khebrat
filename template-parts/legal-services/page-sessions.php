...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const childrenByParent = <?php echo json_encode($children_by_locations); ?>;
    ...
    jQuery(document).ready(function($) {
        function toggleLocationField() {
            var selected = $('input[name="is_remote_session"]:checked').val();
            if (selected === 'no') {
                $('#session-location-wrapper').show();
            } else {
                $('#session-location-wrapper').hide();
            }
        }
        ...
        // نفذ عند التغيير
        $('input[name="is_remote_session"]').on('change', function() {
            toggleLocationField();
        });
    });
    ...
    $('.nextBtn').click(function() {
        ...
        // استخدام smkValidate للتحقق من الحقول المطلوبة داخل الخطوة الحالية
        if (!currentStep.smkValidate()) {
            return; // يمنع الانتقال إذا لم يتم تعبئة الحقول المطلوبة
        }
