    ...
    function exertio_main_menu($theme_location)
    {
        ...
        if (($theme_location) && ($locations = get_nav_menu_locations()) && isset($locations[$theme_location])) {
            ...
            if (isset($menu->term_id)) {
                ...
                $menu_html .= '<li><a href="' . esc_url(home_url( '/' )) . '">' . esc_html__("Home", "khebrat_theme") . '</a></li>';
            }
        } else {
            $menu_html .= '<li><a href="' . esc_url(home_url( '/' )) . '">' . esc_html__("Home", "khebrat_theme") . '</a></li>';
        }
            ...
            foreach (self::$options['fields'] as $field) {
                ...
                if ($field['input_type'] == 'only_text') {
                    ...
                    if ($field['name'] == 'is_megamenu_class') {
                        ...
                        $vals = array(esc_html__("Select Option", "khebrat_theme") => "", esc_html__("Simple Menu", "khebrat_theme") => "simple-menu", esc_html__("Mega Menu", "khebrat_theme") => "mega-menu");
                    ...
                    if ($field['name'] == 'is_megamenu') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="col-12 col-sm-12 col-md-' . esc_attr($size) . ' col-lg-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column size", "khebrat_theme") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_open_side') {
                        $savedValue = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $vals = array(esc_html__("Select Option", "khebrat_theme") => "", esc_html__("Left to Right", "khebrat_theme") => "submenu-left dropdown_menu", esc_html__("Right to Left", "khebrat_theme") => "submenu-right dropdown_menu");
                        $ar = '';
                        foreach ($vals as $val => $name) {
                            $selected = ($savedValue == $name) ? 'selected="selected"' : '';
                            $ar .= '<option value="' . esc_attr($name) . '" ' . $selected . '>' . esc_html($val) . '</option>';
                    ...
                    if ($field['name'] == 'menu_simple_offset') {
                        $savedValue1 = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $ar = '';
                        $ar .= '<option value="">' . esc_html__("Select Option - (No Offset)", "khebrat_theme") . '</option>';
                        for ($size = 12; $size >= 1; $size--) {
                            $selected = ('offset-' . $size == $savedValue1) ? 'selected="selected"' : '';
                            $ar .= '<option value="offset-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column Offset", "khebrat_theme") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_vertical_offset') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="offset-' . esc_attr($size) . '-vertical" ' . $selected . '>' . esc_html__("Vertical Offset", "khebrat_theme") . $size . '</option>';
                        }
                    }
...
if (!function_exists('exertio_item_additional_fields')) {
    function exertio_item_additional_fields($fields)
    {
        $fields['some_text'] = array(
            'name' => 'some_text',
            'label' => '*' . esc_html__('If you want to hide column name in mega menu just write {HideMe} in  Navigation Label', 'khebrat_theme'),
            'input_type' => 'only_text',
        );
        $fields['color'] = array(
            'name' => 'color',
            'label' => esc_html__('Lebel Color', 'khebrat_theme'),
            'container_class' => 'link-color',
            'input_type' => 'color',
        );
        $fields['color_text'] = array(
            'name' => 'color_text',
            'label' => esc_html__('Lebel Text', 'khebrat_theme'),
            'container_class' => 'link-color-text',
            'placeholder' => esc_html__('Enter some text', 'khebrat_theme'),
            'input_type' => 'text',
        );
        $fields['megamenu_class'] = array(
            'name' => 'is_megamenu_class',
            'label' => esc_html__('This select option is only for 1st level menu', 'khebrat_theme'),
            'container_class' => 'megamenu-classes',
            'input_type' => 'select',
        );
        $fields['megamenu'] = array(
            'name' => 'is_megamenu',
            'label' => esc_html__('In parent add bigger value and in 2nd level add small value', 'khebrat_theme'),
            'container_class' => 'megamenu-checkbox',
            'input_type' => 'select',
        );
        $fields['menu_icons'] = array(
            'name' => 'menu_icons',
            'label' => esc_html__('Menu Icons (Font Awesome Icons)', 'khebrat_theme') . 'http://fontawesome.io/icons/',
        ...
        $fields['menu_open_side'] = array(
            'name' => 'menu_open_side',
            'label' => esc_html__('Select Options - For simple menu only', 'khebrat_theme'),
