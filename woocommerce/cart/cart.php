								...
								<div class="woocommerce-cart-form__cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'custom-cart-exertio cart_item d-sm-flex justify-content-between align-items-center mb-3 pb-3 border-bottom', $cart_item, $cart_item_key)); ?>">
									...
									<div class="media d-block d-sm-flex align-items-center <?php echo esc_attr($text); ?>">
										...
										<div class="media-body">
											...
											?>
											<div class="product-price" data-title="<?php echo esc_attr__('Price', 'khebrat_theme'); ?>">
												<?php
											...
											if ($_product->backorders_require_notification() && $_product->is_on_backorder($cart_item['quantity'])) {
												echo wp_kses_post(apply_filters('woocommerce_cart_item_backorder_notification', '<div class="product-badge in-stock backorder_notification">' . esc_html__('Available on backorder', 'khebrat_theme') . '</div>', $product_id));
											}
											?>
										</div>
									</div>
									<div class=" mx-auto mx-sm-0 text-center  <?php echo esc_attr($text); ?>">
										<?php if ($_product->get_type() != "wallet" && $_product->get_type() != "freelancer-packages" && $_product->get_type() != "employer-packages") { ?>
											<div class="product-quantity" data-title="<?php esc_attr_e('Quantity', 'khebrat_theme'); ?>">
												<?php
												if ($_product->is_sold_individually()) {
													$product_quantity = '';
													$product_quantity .= sprintf('<label class="font-weight-medium d-none d-sm-block">%s</label>', esc_html_x('Quantity', 'front-end', 'khebrat_theme'));
													...
													$product_quantity .= sprintf(
														...
														esc_html_x('Quantity', 'front-end', 'khebrat_theme')
													);
										...
										<div class="exertio-product-remove">
											...
											echo apply_filters(
												...
												sprintf(
													...
													esc_html__('Remove this item', 'khebrat_theme'),
													esc_attr($product_id),
													esc_attr($_product->get_sku()),
													esc_html__('Remove', 'khebrat_theme')
												),
												$cart_item_key
											);
											?>
						...
						if ($_product->get_type() != "wallet") { ?>
							<div class="row">
								<?php if (wc_coupons_enabled()) : ?>
									<div class="col-md-6 mb-3">
										<div class="<?php echo esc_attr($margin); ?> input-group">
											<input type="text" name="coupon_code" id="coupon_code" class="form-control" placeholder="<?php echo esc_attr__('Coupon code', 'khebrat_theme'); ?>">
											<div class="input-group-append">
												<button type="submit" class="btn btn-outline-secondary" name="apply_coupon"><?php echo esc_html__('Apply coupon', 'khebrat_theme'); ?></button>
											</div>
								...
								<div class="prop-up-cart col-md-6">
									<button type="submit" class="btn btn-theme btn-block" name="update_cart" value="<?php echo esc_attr__('Update cart', 'khebrat_theme'); ?>">
										<?php echo esc_html__('Update cart', 'khebrat_theme'); ?>
									</button>
								</div>
