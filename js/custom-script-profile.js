(function ($) {
		...
		$.ajax({
			...
			success: function (res) {
				...
				if ($.trim(res_arr[0]) == "1") {
					...
					$(".avatar-profile").html('<img src="' + res_arr[2] + '" class="avatar-img rounded-circle border border-white border-3 shadow" > <i class="mdi mdi-close" id="delete_image" data-post-id="' + pid + '" data-post-meta ="' + post_meta + '" data-attachment-id="' + res_arr[3] + '"></i>');
				}
				else {
	...
	$('#customer_profile_btn').click(function () {
		var this_value = $(this);

		if ($('form#customer_form').smkValidate()) {
			this_value.find('div.bubbles').addClass("view");
			$("#customer_profile_btn").attr("disabled", true);
			var post_id = $(this).attr('data-post-id');
			$.post(localize_vars_frontend.freelanceAjaxurl, { action: 'customer_profile', emp_data: $("form#customer_form").serialize(), post_id: post_id, security: $('#save_pro_nonce').val() }).done(function (response) {
				var get_notification = response.split('|');
				if ($.trim(get_notification[0]) == '1') {
					this_value.find('div.bubbles').removeClass("view");
					toastr.success(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					...
					this_value.find('div.bubbles').removeClass("view");
					$('#customer_profile_btn').attr("disabled", false);
					toastr.error(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
				}
	...
	$('#lawyer_profile_btn').click(function () { 
		var this_value = $(this);

		if ($('form#lawyer_form').smkValidate()) {
			this_value.find('div.bubbles').addClass("view");
			$("#lawyer_profile_btn").attr("disabled", true);
			var post_id = $(this).attr('data-post-id');
			$.post(localize_vars_frontend.freelanceAjaxurl, { action: 'lawyer_profile', emp_data: $("form#lawyer_form").serialize(), post_id: post_id, security: $('#save_pro_nonce').val() }).done(function (response) {
				var get_notification = response.split('|');
				if ($.trim(get_notification[0]) == '1') {
					this_value.find('div.bubbles').removeClass("view");
					toastr.success(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					...
					this_value.find('div.bubbles').removeClass("view");
					$('#lawyer_profile_btn').attr("disabled", false);
					toastr.error(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
				}
	...
	$('#lawyer_info_btn').click(function () { 
		var this_value = $(this);

		if ($('form#lawyer_info_form').smkValidate()) {
			this_value.find('div.bubbles').addClass("view");
			$("#lawyer_info_btn").attr("disabled", true);
			var post_id = $(this).attr('data-post-id');
			$.post(localize_vars_frontend.freelanceAjaxurl, { action: 'lawyer_info', emp_data: $("form#lawyer_info_form").serialize(), post_id: post_id, security: $('#save_info_nonce').val() }).done(function (response) {
				var get_notification = response.split('|');
				if ($.trim(get_notification[0]) == '1') {
					this_value.find('div.bubbles').removeClass("view");
					toastr.success(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					...
					this_value.find('div.bubbles').removeClass("view");
					$('#lawyer_info_btn').attr("disabled", false);
					toastr.error(get_notification[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
				}
	...
	$(document).on('change', '#verification_doc', function () {
		...
		$.ajax({
			...
			success: function (res) {
				...
				if ($.trim(res_arr[0]) == "1") {
					$(".avatar-profile").html('<img src="' + res_arr[2] + '" class="avatar-img rounded-circle border border-white border-3 shadow" >');
					$(".attachment_id").val(res_arr[3]);
				}
				else {
					toastr.error(res_arr[1], '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
...
if (localize_vars_frontend.exertio_notification == true) {
	...
	$('#create_legal_services_btn').click(function () {
		if ($('form#legal_services_form').smkValidate()) {
			$('.loader-outer').show();
			var post_id = $(this).attr('data-post-id');
			$.post(localize_vars_frontend.freelanceAjaxurl, { action: 'legal_services', project_data: $("form#legal_services_form").serialize(), post_id: post_id, security: $('#create_legal_services_nonce').val() }).done(function (response) {
				if (true === response.success) {
					toastr.success(response.data.message, '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					setTimeout(function () {
						window.location.replace(response.data.pid);
...
/*
	$('#create_legal_consultation_btn').click(function () {
		if ($('form#legal_consultation_form').smkValidate()) {
			$('.loader-outer').show();
			var post_id = $(this).attr('data-post-id');
			$.post(localize_vars_frontend.freelanceAjaxurl, { action: 'legal_consultation', project_data: $("form#legal_consultation_form").serialize(), post_id: post_id, security: $('#create_legal_consultation_nonce').val() }).done(function (response) {
				if (true === response.success) {
					toastr.success(response.data.message, '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					setTimeout(function () {
						window.location.replace(response.data.pid);
	...
	$('#create_legal_consultation_btn').click(function () {
		if ($('form#legal_consultation_form').smkValidate()) {
			...
			$.post(localize_vars_frontend.freelanceAjaxurl, { 
				...
				project_data: $("form#legal_consultation_form").serialize(), 
				post_id: post_id, 
				security: $('#create_legal_consultation_nonce').val() 
			}).done(function (response) {
				if (true === response.success) {
					toastr.success(response.data.message, '', { timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod": "slideUp" });
					setTimeout(function () {
