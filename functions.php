<?php if ( ! function_exists( 'khebrat_theme_setup' ) ) :

add_action('after_setup_theme', 'khebrat_theme_setup');
function khebrat_theme_setup() {

    load_theme_textdomain( 'khebrat_theme', get_template_directory() . '/languages' );
 
	
	define('TEMPLATE_THEME_DIR', get_template_directory_uri());
...
function exertio_scripts() {
	...
	function exertio_fonts_url() {
        $fonts_url = '';
        $poppins = _x('on', 'Poppins font: on or off', 'khebrat_theme');
        if ('off' !== $poppins) {
            $font_families = array();
            if ('off' !== $poppins) {
	...
	if ( is_page_template( 'dashboard.php' ) || is_page_template( 'legal-services-form.php' ) )
	{
	...
	if ( is_page_template( 'dashboard.php' ) || is_page_template( 'legal-services-form.php' ) )
	{
		wp_enqueue_script('google-map', '//maps.googleapis.com/maps/api/js?key=' . $khebrat_theme_options['google_map_key'] . '&libraries=places', false, false, true);
	...
	if ( is_page_template( 'page-services-search.php' ) || is_page_template( 'page-project-search.php' ) || is_page_template( 'page-freelancer-search.php' )  || is_page_template( 'dashboard.php' ))
	{
	...
	wp_enqueue_script('script-posts', trailingslashit(get_template_directory_uri()) . 'js/custom-script-posts.js',  array('jquery'), false, true);
	...
	if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
	{
		if(is_page_template( 'dashboard.php' ) && $khebrat_theme_options['zoom_meeting_btn'] == 1)
		{
	...
	if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
	{
		function inline_typography() 
		{
			...
			global $khebrat_theme_options;
			$h2_color = $khebrat_theme_options['opt-typography-body']['color'];
			$main_btn_color = $khebrat_theme_options['opt-theme-btn-color']['regular'];
			$main_btn_color_hover = $khebrat_theme_options['opt-theme-btn-color']['hover'];
			$main_btn_color_shadow = $khebrat_theme_options['opt-theme-btn-shadow-color']['rgba'];
			$main_btn_color_text = $khebrat_theme_options['opt-theme-btn-text-color']['regular'];
			$main_btn_hover_color_text = $khebrat_theme_options['opt-theme-btn-text-color']['hover'];
			
			$sec_btn_color = $khebrat_theme_options['second-opt-theme-btn-color']['regular'];
			$sec_btn_color_hover = $khebrat_theme_options['second-opt-theme-btn-color']['hover'];
			$sec_btn_color_shadow = $khebrat_theme_options['second-opt-theme-btn-shadow-color']['rgba'];
			$sec_btn_color_text = $khebrat_theme_options['second-opt-theme-btn-text-color']['regular'];
			$sec_btn_hover_color_text = $khebrat_theme_options['second-opt-theme-btn-text-color']['hover'];
	...
	if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
	{
		 ...
		 if(is_page_template('page-login.php'))
		 {
			 if(!empty($_GET['key']) && !empty($_GET['login']))
			 {
				  ...
				  if ( is_wp_error($user) )
				  {
					...
					{
						$status_msg = esc_html__('Key is expired.', 'khebrat_theme' );
					}  
					else
					{
						$status_msg = esc_html__('Key is not valid.', 'khebrat_theme' );
					}
				  }
				  else
				  {
					$reset = true;
					$user_id = $user->ID;
				  	$status_msg = esc_html__('Choose your password.', 'khebrat_theme' );
				  }
			 }
		 }
			 ...
			 $activation_status = false;
				if(!empty($_GET['activation_key']))
				{
					...
					if(isset($code) && $code != '' && $code == $data['code'])
					{
						...
						$activation_status_msg = esc_html__('Account Activated Successfully. Please login.', 'khebrat_theme' );
					...
					{
						$activation_status_msg = esc_html__('Activation key is not correct', 'khebrat_theme' );
						$activation_status = false;
					}
				}
		 }
		$exertio_notifications_time = isset($khebrat_theme_options['exertio_notifications_time']) ? $khebrat_theme_options['exertio_notifications_time'] : '';
		...
		wp_localize_script('exertio-theme', 'localize_vars_frontend', array(
			...
			'AreYouSure' => __('Are you sure?','khebrat_theme'),
			'Msgconfirm' => __('Confirmation','khebrat_theme'),
			'remove' => __('Remove','khebrat_theme'),
			'cancel' => __('Cancel','khebrat_theme'),
			'AccDel' => __('Delete, Anyway','khebrat_theme'),
			'proCancel' => __('Cancel, Anyway','khebrat_theme'),
			'confimYes' => __('Yes','khebrat_theme'),
			'confimNo' => __('No','khebrat_theme'),
			'awardDate' => esc_html__('Award Date', 'khebrat_theme'),
			'awardName' => esc_html__('Award Name', 'khebrat_theme'),
			'selectImage' => esc_html__('Image', 'khebrat_theme'),
			'projectURL' => esc_html__('Project url', 'khebrat_theme'),
			'projectName' => esc_html__('Project Name', 'khebrat_theme'),
			'expeName' => esc_html__('Experience Title', 'khebrat_theme'),
			'expeCompName' => esc_html__('Company Name', 'khebrat_theme'),
			'startDate' => esc_html__('Start Date', 'khebrat_theme'),
			'endDate' => esc_html__('End Date', 'khebrat_theme'),
			'endDatemsg' => esc_html__('Leave it empty to set it current job', 'khebrat_theme'),
			'expeDesc' => esc_html__('Description', 'khebrat_theme'),
			'eduName' => esc_html__('Education Title', 'khebrat_theme'),
			'eduInstName' => esc_html__('Institute Name', 'khebrat_theme'),
			'eduEndDatemsg' => esc_html__('Leave it empty to set it current education', 'khebrat_theme'),
			'proAdminCost' => $khebrat_theme_options['project_charges'],
			'YesSure' => esc_html__('Yes, I am sure', 'khebrat_theme'),
			'serviceBuy' => esc_html__('Are you sure you want to purchase this service?', 'khebrat_theme'),
			'maxFaqAllowed' => $khebrat_theme_options['sevices_faqs_count'],
			'maxVideoAllowed' => $khebrat_theme_options['sevices_youtube_links_count'],
			'maxAllowedFields' => esc_html__('Allowed number of fields limit reached', 'khebrat_theme'),
			'faqNo' => esc_html__('FAQ No', 'khebrat_theme'),
			'is_reset' => $is_reset,
			...
			'project_search_link' => isset($khebrat_theme_options['project_search_page']) ? get_the_permalink($khebrat_theme_options['project_search_page']) : '',
			'services_search_link' => isset($khebrat_theme_options['services_search_page']) ? get_the_permalink($khebrat_theme_options['services_search_page']) : '',
			'employer_search_link' => isset($khebrat_theme_options['employer_search_page']) ? get_the_permalink($khebrat_theme_options['employer_search_page']) : '',
			'freelancer_search_link' => isset($khebrat_theme_options['freelancer_search_page']) ? get_the_permalink($khebrat_theme_options['freelancer_search_page']) : '',
			'searchTalentText' => esc_html__('Serach Talent', 'khebrat_theme'),
			'searchEmpText' => esc_html__('Search Employer', 'khebrat_theme'),
			'findJobText' => esc_html__('Find Job', 'khebrat_theme'),
			'searchServiceText' => esc_html__('Get job done', 'khebrat_theme'),
			'is_rtl' => $is_rtl,
			'exertio_local' => $exertio_locale,
			'exertio_notification' => isset($khebrat_theme_options['exertio_notifications']) ? $khebrat_theme_options['exertio_notifications'] : '',
			'notification_time' => $exertio_notifications_time,
			'pass_textWeak' => esc_html__('Weak', 'khebrat_theme'),
			'pass_textMedium' => esc_html__('Medium', 'khebrat_theme'),
			'pass_textStrong' => esc_html__('Strong', 'khebrat_theme'),
			'pass_textVeryStrong' => esc_html__('Very Strong', 'khebrat_theme'),
			)
		);
	}
	if ( is_page_template( 'dashboard.php' ) || is_page_template( 'legal-services-form.php' ) )
...
function get_current_user_id_new(){
    ...
    if( is_super_admin($uid) && $freelancer_id == "" && $employer_id == "" && $customer_id == ""){
...
else if ( is_page_template( 'page-login.php' ) && $khebrat_theme_options['login_footer_show'] == 0){}
else if ( is_page_template( 'page-register.php' ) && $khebrat_theme_options['register_footer_show'] == 0) { }
else 
{
...
if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
{
	if(is_page_template( 'dashboard.php' ) && $khebrat_theme_options['zoom_meeting_btn'] == 1)
	{
        ...
        if( isset($khebrat_theme_options['job_alerts_switch']) && $khebrat_theme_options['job_alerts_switch'] != ""  && $khebrat_theme_options['job_alerts_switch'] == true  ){
...
<input type="hidden" id="nonce_error" value="<?php echo esc_html__('Something went wrong','khebrat_theme'); ?>" />
...
function restrict_site_to_logged_in_users() {
    ...
    // الحصول على معرف ورابط صفحة تسجيل الدخول وصفحة التسجيل
    $login_page_id = isset($khebrat_theme_options['login_page']) ? $khebrat_theme_options['login_page'] : null;
    $register_page_id = isset($khebrat_theme_options['register_page']) ? $khebrat_theme_options['register_page'] : null;
    $register_page_lawyer_id = isset($khebrat_theme_options['lawyer_register_page']) ? $khebrat_theme_options['lawyer_register_page'] : null;

    $login_page_url = $login_page_id ? get_permalink($login_page_id) : wp_login_url();
    $register_page_url = $register_page_id ? get_permalink($register_page_id) : null;

    // التحقق إذا كان المستخدم غير مسجل دخول وليس في لوحة التحكم وليس طلب AJAX
    if (!is_user_logged_in() && !is_admin() && !(defined('DOING_AJAX') && DOING_AJAX)) {
        // منع إعادة التوجيه إذا كان المستخدم في صفحة تسجيل الدخول أو صفحة التسجيل
        if (!is_page([$login_page_id, $register_page_id, $register_page_lawyer_id])) {
