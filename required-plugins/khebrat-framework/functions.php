...
function services_attachment_wp_admin_enqueue() {
	...
	wp_localize_script('attachment_script', 'exertio_localize_vars', array(
		...
		'notification_time' => isset($khebrat_theme_options['exertio_notifications_time']) ? $khebrat_theme_options['exertio_notifications_time'] : '',
	...
	function exertio_database_update_notice(){
		...
		if ( $exists_value == false && $wpdb->get_var("SHOW TABLES LIKE '$table'") != $table && $wpdb->get_var("SHOW TABLES LIKE '$table2'") != $table2) {
			 echo '<div class="notice notice-error exertio-notic">
					<h1> '.__("Exertio WordPress Theme database update needed ." , "khebrat_theme").'</h1>
				 <p>'.__("To use <b>Exertio WordPress Theme</b> propery we must need to update database for the new features and fixes. Please click on button below to update it. " , "khebrat_theme").'</p>
				 <button class="button button-primary" id="update_database">'.__("Update Now" , "khebrat_theme").'</button>
			 </div>';
		}
	}
		...
		<div class="wr-ap">
			...
			<div id="welcome-panel" class="welcome-panel">
				...
				<div class="welcome-panel-contents">
					<h2 style="display: inline;"><?php esc_html_e("Exertio - Freelance Marketplace WordPress Theme ", "khebrat_theme");?>
					</h2>
				...
				<div class="welcome-panel-column-container">
					<div class="welcome-panel-column">
						<h3><?php esc_html_e("Get Started", "khebrat_theme");?></h3>
					   <p>
						<?php esc_html_e("Docementation will helps you to understand the theme flow and will help you to setup the theme accordingly. Click the button below to go to the docementation." , "khebrat_theme");?></p>
						<a class="button button-primary button-hero load-customize hide-if-no-customize" href="https://documentation.scriptsbundle.com/"  target="_blank"><?php esc_html_e("Docementation" , "khebrat_theme");?></a>
					</div>
					<div class="welcome-panel-column">
						<h3><?php esc_html_e("Having Issues? Get Support!", "khebrat_theme");?></h3>
						<p>
						<?php esc_html_e("If you are facing any issue regarding setting up the theme. You can contact our support team they will be very happy to assist you." , "khebrat_theme");?></p>
						<a class="button button-primary button-hero load-customize hide-if-no-customize" href="https://scriptsbundle.ticksy.com/"  target="_blank"><?php esc_html_e("Get Theme Support" , "khebrat_theme");?></a>                    
					</div>
					<div class="welcome-panel-column welcome-panel-last">
						<h3><?php esc_html_e("Looking For Customizations?", "khebrat_theme");?></h3>
						<?php esc_html_e("Looking to add more features in the theme no problem. Our development team will customize the theme according to your requirnments. Click the link below to contact us." , "khebrat_theme");?></p>
						<a class="button button-primary button-hero load-customize hide-if-no-customize" href="https://scriptsbundle.com/freelancer/"  target="_blank"><?php esc_html_e("Looking For Customization?" , "khebrat_theme");?></a>  
					</div>
				</div>
				<br />
				 <p class="hide-if-no-customize">
					<?php esc_html_e("by" , "khebrat_theme");?>, <a href="https://themeforest.net/user/scriptsbundle/portfolio" target="_blank"><?php esc_html_e("ScriptsBundle" , "khebrat_theme");?></a>
				</p>
...
if(!function_exists('exertio_database_update_notice_column'))
{
    function exertio_database_update_notice_column(){
        ...
        if (empty($wpdb->num_rows)) {
            echo '<div class="notice notice-error exertio-notic">
					<h1> ' . __("Exertio WordPress Theme database update needed .", "khebrat_theme") . '</h1>
				 <p>' . __("To use <b>Exertio WordPress Theme</b> properly we must need to update database for the new features and fixes. Please click on button below to update it. ", "khebrat_theme") . '</p>
				 <button class="button button-primary" id="update_database_column">' . __("Update Now", "khebrat_theme") . '</button>
			 </div>';
        }
    }
