...
jQuery("#update_database_column").click( function(e) {
	e.preventDefault();
	...
	jQuery.ajax({
		...
		//dataType : "json",
		url : exertio_localize_vars.ajaxurl,
		data : {action: "update_database_column"},
		success: function(response)
		{
			if(response)
			{
...
/*service rejection*/
jQuery(document).on('click', '#exertio-reject-submit', function (e) {
    'use strict';        
    ...
    jQuery(".adforest-spinner").show();
    var request = jQuery.ajax({
        url: ajaxurl,
        method: "POST",
        data: {post_id: reject_post_id, ad_reject_reason: reject_reason, action: 'fl_ad_rejection',},dataType: "json" });
    request.done(function (response) {jQuery(".adforest-spinner").hide();if (typeof response.status !== 'undefined' && response.status == true) {jQuery('.rej-response').html('<span>' + response.message + '</span>');window.location.href = redirect_url;} else {jQuery('.rej-response').html('<span>' + response.message + '</span>');}});
});