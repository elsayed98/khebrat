    ...
    function fl_authenticate_check($post_id = '')
    {
        ...
        if (get_current_user_id() == "" || get_current_user_id() == 0) {
            if ($post_id == '') {
                ...
                $page = get_the_permalink($khebrat_theme_options['login_page']) . '?redirect=' . $post_id;
...
if (!function_exists('sign_up')) {
    function sign_up()
    {
        ...
        if (isset($params['g-recaptcha-response'])) {
            if (!exertio_recaptcha_verify($khebrat_theme_options['google_recaptcha_secret_key'], $params['g-recaptcha-response'], $_SERVER['REMOTE_ADDR'], '')) {
        ...
        if (email_exists($params['fl_email']) == false) {
            if ($wpdb->get_row("SELECT post_name FROM " . $base_prefix . "posts WHERE post_name = '" . $params['fl_username'] . "'", 'ARRAY_A')) {
                ...
                if (isset($redirect) && $redirect != '') {
                    ...
                    $page = get_the_permalink($khebrat_theme_options['user_dashboard_page']);
                }
                if (fl_framework_get_options('fl_allow_user_email_verification') == false) {
                    ...
                    if (fl_framework_get_options('fl_user_email_verification') == false) {
                        exertio_account_activation_email($uid);
                    }
                    $page = get_the_permalink($khebrat_theme_options['registration_page']);
...
if (!function_exists('exertion_on_registration_funtion')) {
    function exertion_on_registration_funtion($uid)
    {
        ...
        if (isset($khebrat_theme_options['user_registration_type']) && $khebrat_theme_options['user_registration_type'] == 'both') {
            if (function_exists('exertio_register_type_return')) {
                exertio_register_type_return($uid, 'both');
            }
        } else if (isset($khebrat_theme_options['user_registration_type']) && $khebrat_theme_options['user_registration_type'] == 'both_selected') {
            if (function_exists('exertio_register_type_return')) {
                if (isset($khebrat_theme_options['user_registration_type_selection']) && count(array_filter($khebrat_theme_options['user_registration_type_selection'])) < 2) {
                    $user_type = $khebrat_theme_options['user_registration_type_selection']['0'];
                } else {
                    global $wp_session;
        ...
        if (function_exists('exertio_generate_code_registeration')) {
            if (fl_framework_get_options('fl_user_email_verification') !== null && fl_framework_get_options('fl_user_email_verification') == true && fl_framework_get_options('fl_allow_user_email_verification') == false) {
                //exertio_generate_code_registeration($uid);
                exertio_account_activation_email($uid);
            }
        }
            ...
            if (exertio_recaptcha_verify($khebrat_theme_options['google_recaptcha_secret_key'], $params['g-recaptcha-response'], $_SERVER['REMOTE_ADDR'], '')) {
                ...
                if (!is_wp_error($user)) {
                    if (count((array) $user->roles) == 0) {
                        ...
                        if ($uid) {
                            $user_redirection_after_login = $khebrat_theme_options['user_redirection_after_login'];
        ...
        if (!empty($field_name)) {
            ...
            foreach ($files['name'] as $key => $value) {
                if ($files['name'][$key]) {
                    ...
                    // Check file size
                    $image_size = $khebrat_theme_options['user_attachment_size'];
...
function fl_employer_profile()
{
    ...
    if ($post_author == $uid) {
        ...
        $words = explode(',', $khebrat_theme_options['bad_words_filter']);
        $replace = $khebrat_theme_options['bad_words_replace'];
...
add_action('wp_ajax_customer_profile', 'fl_customer_profile');
function fl_customer_profile()
{
    /*DEMO DISABLED*/
    exertio_demo_disable('echo');
    check_ajax_referer('fl_save_pro_secure', 'security');
    ...
    if ($post_author == $uid) {
        ...
        if (isset($params['customer_full_name'])) {
            update_post_meta($post_id, '_customer_full_name', sanitize_text_field($params['customer_full_name']));
        }
        if (isset($params['customer_gender'])) {
            update_post_meta($post_id, '_customer_gender', sanitize_text_field($params['customer_gender']));
        }

       
        
        if (isset($params['customer_position'])) {
            update_post_meta($post_id, '_customer_position', sanitize_text_field($params['customer_position']));
        }
        if (isset($params['customer_attribute'])) {
            update_post_meta($post_id, '_customer_attribute', sanitize_text_field($params['customer_attribute']));
        }
        if (isset($params['company_name'])) {
            update_post_meta($post_id, '_company_name', sanitize_text_field($params['company_name']));
        }
        if (isset($params['company_tax'])) {
            update_post_meta($post_id, '_company_tax', sanitize_text_field($params['company_tax']));
        }
       
        if (isset($params['date_birth_id'])) {
            update_post_meta($post_id, '_customer_date_birth', sanitize_text_field($params['date_birth_id']));
        }
        if (isset($params['customer_portfolio_link'])) {
            update_post_meta($post_id, '_customer_portfolio_link', sanitize_text_field($params['customer_portfolio_link']));
        }



        if (isset($params['customer_location'])) {
            $customer_location_terms = array((int)$params['customer_location']);
            update_post_meta($post_id, '_customer_location', $params['customer_location']);
            wp_set_post_terms($post_id, $customer_location_terms, 'customer-locations', false);
        }

        
        if (isset($params['banner_img_id'])) {
            update_post_meta($post_id, '_customer_banner_id', sanitize_text_field($params['banner_img_id']));
        }

        if (isset($params['customer_address'])) {
            update_post_meta($post_id, '_customer_address', sanitize_text_field($params['customer_address']));
        }

        if (isset($params['customer_latitude'])) {
            update_post_meta($post_id, '_customer_latitude', sanitize_text_field($params['customer_latitude']));
        }

        if (isset($params['customer_longitude'])) {
            update_post_meta($post_id, '_customer_longitude', sanitize_text_field($params['customer_longitude']));
        }
...
if (!function_exists('freelance_project_attachments')) {
    function freelance_project_attachments()
    {
        ...
        if (isset($khebrat_theme_options['project_attachment_count'])) {
            $condition_img = $khebrat_theme_options['project_attachment_count'];
        }
        if (isset($khebrat_theme_options['project_attachment_size'])) {
            $attachment_size = $khebrat_theme_options['project_attachment_size'];
        }
        if (!empty($field_name)) {
            require_once ABSPATH . 'wp-admin/includes/image.php';
            require_once ABSPATH . 'wp-admin/includes/file.php';
    ...
    function fl_create_project()
    {
        ...
        if ($params['is_update'] != '') {
            if ($project_status == 'publish') {
                $status = 'publish';
                if (isset($khebrat_theme_options['update_project_approval']) && $khebrat_theme_options['update_project_approval'] == 0) {
                    $status = 'pending';
                }
            } else {
                $status = 'pending';
            ...
            if (isset($khebrat_theme_options['project_approval']) && $khebrat_theme_options['project_approval'] == 0) {
        ...
        $page_link = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . "?ext=create-project&pid=" . $post_id;
    ...
    function fl_addon_save()
    {
        ...
        if ($params['is_update'] != '') {
            if ($addon_status == 'publish') {
                $status = "publish";
                if (isset($khebrat_theme_options['addons_update_approval']) && $khebrat_theme_options['addons_update_approval'] == 0) {
                    $status = "pending";
                }
            } else {
                $status = "pending";
            ...
            $status = "publish";
            if (isset($khebrat_theme_options['addons_approval']) && $khebrat_theme_options['addons_approval'] == 0) {
                $status = "pending";
            }
        }
        $words = explode(',', $khebrat_theme_options['bad_words_filter']);
        ...
        $page_link = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . "?ext=create-addon&aid=" . $post_id;
        ...
        if(isset($khebrat_theme_options['addons_approval']) &&  $khebrat_theme_options['addons_approval'] == 0 || isset($khebrat_theme_options['addons_update_approval']) &&  $khebrat_theme_options['addons_update_approval'] == 0){
            $return = array('message' => esc_html__('Addon created and waiting for admin approval', 'exertio_framework'), 'pid' => $page_link);
            /*Mail to Admin for Addon Approval*/
           echo fl_addon_created_admin_email($uid,$post_id);

        } else {
             $return = array('message' => esc_html__('Addon Created Succesfully', 'exertio_framework'), 'pid' => $page_link);
        }
        wp_send_json_success($return);
    }
}
...
if (!function_exists('freelance_services_attachments')) {
    function freelance_services_attachments()
    {
        ...
        $attachment_size = '2000';
        $img_count = count(array_count_values($field_name['name']));
        if (isset($khebrat_theme_options['sevices_attachment_count'])) {
            $condition_img = $khebrat_theme_options['sevices_attachment_count'];
        }
        if (isset($khebrat_theme_options['services_attachment_size'])) {
            $attachment_size = $khebrat_theme_options['services_attachment_size'];
        }
        if (!empty($field_name)) {
            require_once ABSPATH . 'wp-admin/includes/image.php';
            require_once ABSPATH . 'wp-admin/includes/file.php';
...
if (!function_exists('fl_service_save')) {
    function fl_service_save()
    {
        ...
        global $khebrat_theme_options;
        /*CHECK IF EMIL IS VERIFIED*/
        if (isset($khebrat_theme_options['services_with_email_verified']) && $khebrat_theme_options['services_with_email_verified'] == 0) {
        ...
        if ($params['is_update'] != '') {
            if ($service_status == 'publish') {
                $status = "publish";
                if (isset($khebrat_theme_options['service_update_approval']) && $khebrat_theme_options['service_update_approval'] == 0) {
                    $status = "pending";
                }
            } else {
                $status = "pending";
            ...
            if (isset($khebrat_theme_options['service_approval']) && $khebrat_theme_options['service_approval'] == 0) {
        ...
        $page_link = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . "?ext=add-services&sid=" . $post_id;
        if ($params['is_update'] == '') {
            ...
            if (isset($project_credits) && $project_credits > 0 || $project_credits == -1 || $free_proposals == 1) {
                ...
                if ($current_user_id != $post->post_author) {
                    ...
                    if (empty($result)) {
                        ...
                        $p_charges = $khebrat_theme_options['project_charges'];
                        ...
                        if (isset($params['top_bid']) || isset($params['sealed_bid']) || isset($params['featured_bid'])) {
                            ...
                            if (isset($params['top_bid'])) {
                                $top_bid_charges = $khebrat_theme_options['project_top_addon_price'];
                                $is_top = '1';
                            }
                            if (isset($params['sealed_bid'])) {
                                $sealed_bid_charges = $khebrat_theme_options['project_sealed_addon_price'];
                                $is_sealed = '1';
                            }
                            if (isset($params['featured_bid'])) {
                                $featured_bid_charges = $khebrat_theme_options['project_featured_addon_price'];
                                $is_featured = '1';
                            }
...
if (!function_exists('fl_verification_save')) {
    function fl_verification_save()
    {
        ...
        $page_link = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . "?ext=identity-verification";
...
function fl_send_proposal_offer_modal()
{
    ...
    exertio_demo_disable('json');
    check_ajax_referer('fl_gen_secure', 'security');
    global $khebrat_theme_options, $wpdb;
...
function fl_accept_project_offer_callback()
{
    ...
    if ($status_val == "2") {
        ...
        if (is_wp_error($result)) {
            ...
            $redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . '?ext=ongoing-project-proposals&project-id=' . $pid;
                                        ...
                                        $html_modal .= '<div class="form-group">
																							...
																							<div class="input-group">
																							  ...
																							  <div class="input-group-prepend">
																								<div class="input-group-text">' . esc_html($khebrat_theme_options['fl_currency']) . '</div>
																							  </div>
																							</div>
																						  </div>';
                                    } else if ($project_type == 'hourly' || $project_type == 2) {
																						  ...
																						  <div class="col-12">
																							<div class="fr-project-adons w1">
																							  <ul>
																								...
																								<li> <span>' . esc_html($khebrat_theme_options['project_top_addon_title']) . '</span>
																								  <p>' . esc_html($khebrat_theme_options['project_top_addon_desc']) . '</p>
																								</li>
																								<li> <span>' . esc_html(fl_price_separator($khebrat_theme_options['project_top_addon_price'])) . '</span> </li>
																							  </ul>
																								<div class="bottom-icon">
																									' . wp_return_echo($khebrat_theme_options['project_top_addon_icon']) . '
																								</div>
																							</div>
																						  </div>
																						</div>';
																						  ...
																						  <div class="col-12">
																							<div class="fr-project-adons w2">
																							  <ul>
																								...
																								<li> <span>' . esc_html($khebrat_theme_options['project_sealed_addon_title']) . '</span>
																								  <p>' . esc_html($khebrat_theme_options['project_sealed_addon_desc']) . '</p>
																								</li>
																								<li> <span>' . esc_html(fl_price_separator($khebrat_theme_options['project_sealed_addon_price'])) . '</span> </li>
																							  </ul>
																								<div class="bottom-icon">' . wp_return_echo($khebrat_theme_options['project_sealed_addon_icon']) . '</div>
																							</div>
																						  </div>
																						</div>';
                                            }
																						  ...
																						  <div class="col-12">
																							<div class="fr-project-adons w3">
																							  <ul>
																								...
																								<li> <span>' . esc_html($khebrat_theme_options['project_featured_addon_title']) . '</span>
																								  <p>' . esc_html($khebrat_theme_options['project_featured_addon_desc']) . '</p>
																								</li>
																								<li> <span>' . esc_html(fl_price_separator($khebrat_theme_options['project_featured_addon_price'])) . '</span> </li>
																							  </ul>
																								<div class="bottom-icon">' . wp_return_echo($khebrat_theme_options['project_featured_addon_icon']) . '
																								</div>
																							</div>
																						  </div>
																						</div>';
																				  ...
																				  <div class="col-12">
																					<div class="button-bid">
																					  ...
																					  <div>
																						...
																						</div> <div>' . esc_html__('I agree to the ', 'exertio_framework') . '<a href="' . esc_url($khebrat_theme_options['bid_tems_link']) . '">' . esc_html__('terms and conditions', 'exertio_framework') . '</a></div> </div>
    ...
    function fl_detail_search_page()
    {
        ...
        if (fl_framework_get_options('project_location') == 3) {
            ...
            if (!empty($project_location) && !is_wp_error($project_location) || $location_remote != '' && $location_remote != 0) {
                if (isset($location_remote) && $location_remote == 1) {
                    $project_loc = esc_html__('Remote', 'khebrat_theme');
                } else {
        ...
        if (fl_framework_get_options('project_freelancer_type') == 3) {
            ...
            $features .= '<li>
                ...
                <div class="ftr-txt">
                    <p>' . esc_html__('Freelancer Type ', 'khebrat_theme') . '</p>
                    <h4>
                        ' . $freelancer_name . '
                    </h4>
                </div>
        ...
        if (fl_framework_get_options('project_duration') == 3) {
            ...
            $features .= '<li>
                ...
                <div class="ftr-txt">
                    <p>' . esc_html__('Project Duration', 'khebrat_theme') . '</p>
                    <h4> ' . $proj_duration . '</h4>
                </div>
            </li>';
        }
        ...
        if (fl_framework_get_options('project_level') == 3) {
            ...
            $features .= '  <li>
                ...
                <div class="ftr-txt">
                    <p>' . esc_html__('Level', 'khebrat_theme') . '</p>
                    <h4>' . $proj_level . '</h4>
                </div>
            </li>';
        }
        ...
        if (fl_framework_get_options('project_english_level') == 3) {
            ...
            $features .= '<li>
                ...
                <div class="ftr-txt">
                    <p>' . esc_html__('English Level ', 'khebrat_theme') . '</p>
                    <h4>' . $proj_english . '</h4>
                </div>
            </li>';
        }
        ...
        if (fl_framework_get_options('project_languages') == 3) {
            ...
            $features .= '<li>
                ...
                <div class="ftr-txt">
                    <p>' . esc_html__('Languages', 'khebrat_theme') . '</p>
                    <h4>' . $sav_language . '</h4>
                </div>
            </li>';
        }
        ...
        if (fl_framework_get_options('project_skills') == 3) {
            ...
            $skills .= '<div class="fr-project-skills">
                <h3>' . esc_html__('Skills Required', 'khebrat_theme') . '</h3>
                ' . $Skills_detail . '
            </div>';
        }
        if ($pid != '') {
            ...
            $html .= ' <div class="detail_loader" style="position: relative">
                ...
                <div class="exer-fr-dtl">
                    ...
                    <div class="about-fr-descrip">
                        <h3>' . esc_html__('Description', 'khebrat_theme') . '</h3>
                        ' . $content_all . '
                    </div>
                      ' . $skills . '
                    <div class="dtl-btn">
                        <a class="view-btn btn-theme" target="_blank" href="' . esc_url(get_the_permalink($pid)) . '">' . esc_html__('View Detail', 'khebrat_theme') . '</a>
                    </div>
                </div>';
...
if (!function_exists('fl_earning')) {
    function fl_earning()
    {
        ...
        if (isset($active_profile) && $active_profile == 2) {
            ...
            $html = '';
            if (isset($khebrat_theme_options['freelancer_show_earning']) && $khebrat_theme_options['freelancer_show_earning'] == 1) {
            ...
            $html = '';
            if (isset($khebrat_theme_options['employer_show_spending']) && $khebrat_theme_options['employer_show_spending'] == 1) {
                $html .= ' <div class="detail_loader_earning" style="position: relative">
                <div class="loader-outer" style="display: none;">
                    <div class="loading-inner">
                        <div class="loading-inner-meta">
            ...
            <div class="metric">
                ...
                <p>
                    <span class="title">' . esc_html__('Employer Spendings', 'khebrat_theme') . '</span>
                    <span class="number">' . esc_html($earned_amount_symbol) . '</span>
                </p>
            </div>
            <p class="matric-bottom"> <a href="#">' . esc_html__('View Detail', 'khebrat_theme') . ' <i class="fas fa-arrow-right"></i></a></p>
        </div>';
            }
            $return = array('html' => $html);
            wp_send_json_success($return);
    ...
    function fl_ad_rejection_callback()
    {
        ...
        if ($post_type == "services") {
            ...
            if (fl_framework_get_options('fl_email_onservice_rejected')) {
                ...
                $body = str_replace($msg_keywords, $msg_replaces, $khebrat_theme_options['fl_onservice_rejected_body']);
            ...
            if (fl_framework_get_options('fl_email_onservice_rejected')) {
                ...
                $msg_keywords = array('%display_name%', '%site_name%', '%service_title%', '%service_link%', '%rejection_reason%');
                $msg_replaces = array($user_info->display_name, get_bloginfo('name'), get_the_title($rej_ad_id), get_the_permalink($rej_ad_id), $ad_reject_reason);
                $body = str_replace($msg_keywords, $msg_replaces, $khebrat_theme_options['fl_on_project_rejected_body']);
                $body = stripcslashes($body);
                if (wp_mail($to, $subject, $body, $headers)) {
                    $status['status'] = true;
                    $status['message'] = __('Email sent to the ad author successfully', 'exertio_framework');
...
add_action('wp_ajax_sign_up', 'customer_sign_up'); 
add_action('customer_sign_up', 'customer_sign_up');
if (!function_exists('customer_sign_up')) {
...
add_action('wp_ajax_legal_services', 'fl_legal_services');
if (!function_exists('fl_legal_services')) {
    function fl_legal_services()
    {
        /*DEMO DISABLED*/
        exertio_demo_disable('json');
        check_ajax_referer('fl_create_legal_services_secure', 'security');
        $current_user_id = get_current_user_id();
        global $khebrat_theme_options;
        /*CHECK IF EMIL IS VERIFIED*/
        /*
        ...
        if (isset($params['locations_child_term'])) {
           

            $terms = array_map('intval', $params['locations_child_term']);
            
            // حفظ التصنيفات
            wp_set_object_terms($post_id, $terms, 'legal_category');
        }
        /*
        if (isset($params['project_lat'])) {
            update_post_meta($post_id, '_project_latitude', sanitize_text_field($params['project_lat']));
         ...
         */
        if (isset($params['study_related'])) {
            update_post_meta($post_id, '_study_related', sanitize_text_field($params['study_related']));
        }


        if (isset($params['case_role_type'])) {
			update_post_meta($post_id, '_case_role_type', sanitize_text_field($params['case_role_type']));
		}
	
		if (isset($params['plaintiff_identity_type'])) {
			update_post_meta($post_id, '_plaintiff_identity_type', sanitize_text_field($params['plaintiff_identity_type']));
		}
	
		if (isset($params['defendant_identity_type_by_plaintiff'])) {
			update_post_meta($post_id, '_defendant_identity_type_by_plaintiff', sanitize_text_field($params['defendant_identity_type_by_plaintiff']));
		}
	
		if (isset($params['defendant_identity_type'])) {
			update_post_meta($post_id, '_defendant_identity_type', sanitize_text_field($params['defendant_identity_type']));
		}
	
		if (isset($params['plaintiff_identity_type_by_defendant'])) {
			update_post_meta($post_id, '_plaintiff_identity_type_by_defendant', sanitize_text_field($params['plaintiff_identity_type_by_defendant']));
		}


		if (isset($params['request_title'])) {
			update_post_meta($post_id, '_request_title', sanitize_text_field($params['request_title']));
		}
		
		if (isset($params['case_subject'])) {
			update_post_meta($post_id, '_case_subject', sanitize_textarea_field($params['case_subject']));
		}
		
		if (isset($params['case_questions'])) {
			update_post_meta($post_id, '_case_questions', sanitize_textarea_field($params['case_questions']));
		}



        update_user_meta($current_user_id, '_processing_post_id', '');
        update_post_meta($post_id, '_legal_services_status', 'active');
        /*ATTACHMENT UPDATED*/
        update_post_meta($post_id, '_project_attachment_ids', $params['project_attachment_ids']);
        $selected_reference = '';
        if (isset($post_id) && $post_id != "") {
