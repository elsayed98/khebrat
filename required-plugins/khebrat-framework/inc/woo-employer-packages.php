...
add_action( 'woocommerce_thankyou', 'exertio_employer_package_auto_complete_order' );
	...
	if ( ! function_exists( 'exertio_employer_pck_on_registeration' ) )
	{
		...
		{ 
			...
			if(isset($khebrat_theme_options['employer_default_package_switch']) && $khebrat_theme_options['employer_default_package_switch'] == 1 )
			{
				if(isset($khebrat_theme_options['employer_default_packages']) && $khebrat_theme_options['employer_default_packages'] != '' )
				{
					$product_id = $khebrat_theme_options['employer_default_packages'];
					$product = wc_get_product( $product_id );
					if(!empty($product))
					{
						$simple_projects = get_post_meta($product_id, '_simple_projects', true);
