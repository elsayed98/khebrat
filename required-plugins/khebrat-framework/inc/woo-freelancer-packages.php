...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) 
{
	...
	if ( ! function_exists( 'exertio_freelancer_pck_on_registeration' ) )
	{
		...
		{ 
			...
			if(isset($khebrat_theme_options['freelancer_default_package_switch']) && $khebrat_theme_options['freelancer_default_package_switch'] == 1 )
			{
				if(isset($khebrat_theme_options['freelancer_default_packages']) && $khebrat_theme_options['freelancer_default_packages'] != '' )
				{
					$product_id = $khebrat_theme_options['freelancer_default_packages'];
					$product = wc_get_product( $product_id );
					if(!empty($product))
					{
						$project_credits = get_post_meta( $product_id, '_project_credits', true );
