...
if ( ! function_exists( 'exertio_get_username' ) ) 
{
	function exertio_get_username( $user_type, $user_id = '', $show_badge = '', $badge_position = 'left')
	{
		if($user_id == '')
		{
			...
			$cus_id = get_user_meta( $current_user_id, 'customer_id' , true );
		}
		else
		{
			$emp_id = $user_id;
		...
		if($user_type == 'customer')
		{
			if($show_badge == 'badge')
			{
				$verifed_badge = get_post_meta( $cus_id, '_is_customer_verified' , true );
				if(isset($verifed_badge) && $verifed_badge == 1)
				{
					$verified = '<i class="fa fa-check verified protip" data-pt-position="top" data-pt-scheme="black" data-pt-title="'.esc_html__( 'Verified', 'exertio_framework' ).'"></i> ';	
				}
			...
			$user_name = get_post_meta( $cus_id, '_customer_full_name' , true );
			if($user_name == '')
			{
				$post	=	get_post($cus_id);
				if($post != null)
				{
					$user_name = $post->post_title;
				}
...
//				return $image;
        ...
        {
            return '<a href="'.get_the_permalink($id) .'"><img src="'.esc_url($khebrat_theme_options['services_default_img']['url']).'" alt="'.esc_attr(get_post_meta($alt_id, '_wp_attachment_image_alt', TRUE)).'" class="img-fluid"></a>';
        }
    }
}

if ( ! function_exists( 'exertio_get_service_post_image_slider' ) )
{
	function exertio_get_service_post_image_slider($id)
	{
		global $khebrat_theme_options;
		$alt_id= '';
		$services_img_id ='';
...
if (!function_exists('exertio_randomString')) {
    ...
    function exertio_randomString($length = 5)
    {
        $str = "";
