	...
	function fl_assign_project()
	{
		...
		if( $pid != '' && $fl_id != '')
		{
			...
			if($author_id == $current_user_id)
			{
				...
				if(isset($khebrat_theme_options['allow_project_propsal_amount']) && $khebrat_theme_options['allow_project_propsal_amount'] == 1){
					...
					else
					{
						...
						if($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table)
						{
							...
							if($log_update_id)
							{
								...
								$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=completed-project-detail&project-id='.$pid;
			...
			else if($status == 'cancel')
			{
				$project_status = "'canceled'";
					...
					else
					{
						...
						$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=canceled-projects';
	...
	function fl_simple_expire_project()
	{
		...
		if(get_current_user_id() == $post_author)
		{
			...
			else
			{
				...
				$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=expired-project';
	...
	function fl_simple_reactive_project()
	{
		...
		if(get_current_user_id() == $post_author)
		{
			...
			else
			{
				...
				$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=project';
					...
					if($update_id)
					{
						...
						if($review_id)
						{
							...
							if($wpdb->get_var("SHOW TABLES LIKE '$log_table'") == $log_table)
							{
								...
								if($log_update_id)
								{
									$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=completed-service-detail&sid='.$ongoing_sid;
			...
			if($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table)
			{
				...
				if($result[0]['buyer_id'] == $employer_id)
				{
					...
					if($update_id)
					{
						...
						$redirect_page = get_the_permalink($khebrat_theme_options['user_dashboard_page']).'?ext=canceled-service-detail&sid='.$ongoing_sid;
		...
		if($the_query->have_posts())
		{
			...
			while ( $the_query->have_posts() ) 
			{
				...
				if($project_type == 'hourly' || $project_type == 2)
				{
					...
					$type = '<p class="price_type protip" data-pt-title="'.__('For ','exertio_framework').$hours.__(' hours total will be  ','exertio_framework'). fl_price_separator($hours*$price).'" data-pt-position="top" data-pt-scheme="black">'.esc_html__( 'Hourly', 'khebrat_theme' ).' <i class="far fa-question-circle"></i></p>';
				}
				else if($project_type == 'fixed' || $project_type == 1)
				{
					$type = '<p class="price_type ">'.esc_html__( 'Fixed', 'khebrat_theme' ).'</p>';
				}
...
{ 
	function exertio_user_switch()
	{
		...
		else if(isset($active_profile) && $active_profile == 'customer' )
		{
...
if (!function_exists('exertio_job_alert_paid_subscription')) {
	...
	function exertio_job_alert_paid_subscription() {
		...
		if ($woocommerce->cart->add_to_cart($product_id, 1)) {
			echo '1|' . __("Added to cart.", 'khebrat_theme') . '|' . wc_get_cart_url();
		} else {
			echo '1|' . __("Already in your cart.", 'khebrat_theme') . '|' . wc_get_cart_url();
		}
		die();
	}
...
if ( ! function_exists( 'delete_payout' ) )
{
	function delete_payout()
	{
		...
		if(isset($uid) && $uid != '') {
			...
			$page_link = get_the_permalink($khebrat_theme_options['user_dashboard_page']) . "?ext=payouts";
