...
if ( ! function_exists( 'fl_get_all_services' ) )
{
	function fl_get_all_services()
	{
        ...
        $table = EXERTIO_PURCHASED_SERVICES_TBL;
            $query = "SELECT DISTINCT service_id FROM ".$table." WHERE
             (`status` IN ('ongoing', 'completed', 'canceled') AND `buyer_id` = $fl_id) OR 
             (`status` IN ('ongoing', 'completed', 'canceled') AND `seller_id` = $seller_id) ORDER BY timestamp DESC";
		...
		if(isset($status) && $status == 'publish' || isset($status) && $status == 'completed' || isset($status) && $status == 'canceled')
		{
			...
			if($service_id != '' || $dispute_title != '' || $desc != '')
			{
				if($is_disputed == '')
				{
                    ...
                    if(fl_framework_get_options('fl_email_on_dispute_service') == true)
                    {
                        fl_framework_email_to_admin_dispute_service($dispute_id,$against_id,$disputer_id,$service_id);
                    }
