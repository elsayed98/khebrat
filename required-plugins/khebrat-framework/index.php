<?php
/**
 * Plugin Name: Khebrat Framework
 * Plugin URI: #
 * Description: khebrat framework is essential for the proper khebrat WordPress Theme funcationality.
 * Version: 1.2.6
 * Author: Scripts Bundle
 * Author URI: #
 * License: GPL2
 * Text Domain: khebrat_framework
 */
$my_theme = wp_get_theme();
if ($my_theme->get('Name') != 'Khebrat' && $my_theme->get('Name') != 'Khebrat child')
{
   return;
}
else
{
	...
	require FL_PLUGIN_PATH . 'plugin-files/cpt/khebrat-customer.php';
	...
	if ( ! function_exists( 'exertio_framework_chart_strings' ) )
	{
		function exertio_framework_chart_strings()
		{
			if(is_singular('projects') || is_singular('services') || is_singular('employer') || is_singular('freelancer') || isset($_GET['page-type']) && $_GET['page-type'] == 'dashboard' || is_page_template('page-profile.php') && empty($_GET['page-type']))
			{
				...
				$is_show = isset( $khebrat_theme_options['exertio_layout_manager']['enabled']['views']) ? '1' : '0';
				$data = $labes = '';
				$chart_type = 'bar';
				if(isset($khebrat_theme_options['exertio_chart_type']) && $khebrat_theme_options['exertio_chart_type'] !="")
				{
					$chart_type = $khebrat_theme_options['exertio_chart_type'];
				}
				$chart_bg =  isset($khebrat_theme_options['exertio_chart_bg']['rgba']) ? $khebrat_theme_options['exertio_chart_bg']['rgba'] : 'rgba(0,174,239,0.2)';
				$chart_border =  isset($khebrat_theme_options['exertio_chart_border']) ? $khebrat_theme_options['exertio_chart_border'] : '#00aeef';
